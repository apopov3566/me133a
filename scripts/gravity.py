...
class Kinematics:
    def __init__(self, robot, baseframe, tipframe):
        ...
        rospy.loginfo(
            "Kinematics: Setting up the chain from '%s' to '%s'...", baseframe, tipframe
        )
        ...
        rospy.loginfo(
            "Kinematics: %d active DOFs, %d total steps", self.dofs, len(self.joints)
        )
    ...
    def grav(self, theta):
        ...
        for (joint, link) in zip(self.joints, self.links):
            ...
            if link.inertial is not None:
                for i in range(index):
                    grav[i] += np.dot(
                        link.inertial.mass * g,
                        np.cross(
                            elist[i],
                            (
                                p_from_T(T @ T_from_URDF_origin(link.inertial.origin))
                                - plist[i]
                            ),
                            axis=0,
                        ),
